...
function Content() {
    ...
    const load = useCallback(async () => { setLoading(true); try {
        ...
        if (!r.ok || !o.ok || !d.ok)
            throw Error(j.error?.message ?? oj.error?.message ?? dj.error?.message ?? "Unable to load timetable options");
        setRows(j.data);
        setMeta(j.meta);
        setOpts(oj.data);
        setStats(dj.data);
    ...
    const visible = view === "daily" ? rows.filter(r => r.day === (day || days[(new Date().getDay() + 6) % 7])) : rows, chosenBatch = opts.batches.find(x => x.id === form.batchId), formCourses = Array.from(new Map(opts.batches.filter(x => x.branchId === form.branchId && x.courseId).map(x => [x.courseId, x.course])).entries()).map(([id, course]: any) => ({ id, ...course })), formBatches = opts.batches.filter(x => x.branchId === form.branchId && x.courseId === form.courseId), formSubjects = opts.subjects.filter(x => x.courseId === form.courseId), formTeachers = opts.teachers.filter(x => x.branchId === form.branchId), rooms = opts.classrooms.filter(x => x.branchId === form.branchId), chooseConfiguredPeriod = (periodNumber: number) => { const period = configuredPeriods.find(item => item.periodNumber === periodNumber); if (period) setForm(current => ({ ...current, periodNumber: period.periodNumber, startTime: `${String(Math.floor(period.startMinute / 60)).padStart(2, "0")}:${String(period.startMinute % 60).padStart(2, "0")}`, endTime: `${String(Math.floor(period.endMinute / 60)).padStart(2, "0")}:${String(period.endMinute % 60).padStart(2, "0")}` })); };
