...
export default function Page() {
  ...
  >
    {!platform ? <div className="mt-6 rounded-xl border border-amber-200 bg-amber-50 p-5 text-sm text-amber-900">
    ...
    </div> : <div className="implementation-kit-print mt-6 space-y-6">
      <section className="implementation-kit-print-section card p-5">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          ...
          <button type="button" onClick={() => window.print()} className="implementation-kit-print-hide inline-flex items-center gap-2 rounded-lg border px-3 py-2 text-sm font-semibold">
            <Printer size={16}/>Print implementation checklist
          </button>
        </div>
        <div className="implementation-kit-print-grid mt-4 grid gap-3 md:grid-cols-2">
          {templates.map(template => <article key={template.file} className="implementation-kit-print-card rounded-xl border p-4">
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <a href={template.file} download className="implementation-kit-print-hide mt-3 inline-flex items-center gap-2 text-sm font-semibold text-brand-700"><Download size={15}/>Download CSV template</a>
              </div>
            </div>
          </article>)}
        </div>
      ...
      <section className="implementation-kit-print-section card p-5">
        ...
        <div className="implementation-kit-print-grid mt-4 grid gap-2 md:grid-cols-2">
          {implementation.map((item, index) => <div key={item} className="implementation-kit-print-item flex gap-3 rounded-lg border p-3 text-sm">
      ...
      <section className="implementation-kit-print-section card p-5">
        ...
        <div className="implementation-kit-print-grid mt-4 grid gap-2 md:grid-cols-2">
          {uat.map(item => <div key={item} className="implementation-kit-print-item flex items-start gap-2 rounded-lg border p-3 text-sm"><CheckCircle2 size={17} className="mt-0.5 shrink-0 text-slate-400"/><span>{item}</span></div>)}
        </div>
      ...
      <section className="implementation-kit-print-grid grid gap-6 lg:grid-cols-2">
        <article className="implementation-kit-print-card card p-5">
      ...
      <section className="implementation-kit-print-section card p-5">
        ...
        <div className="implementation-kit-print-hide mt-4 flex flex-wrap gap-3">
