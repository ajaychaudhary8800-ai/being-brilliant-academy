...
function Field({ label, value, onChange, type = "text", help, minLength, maxLength, pattern, placeholder }: { label: string; value: string; onChange: (value: string) => void; type?: string; help?: string; minLength?: number; maxLength?: number; pattern?: string; placeholder?: string }) { return <label className="text-sm font-semibold">{label}<input required type={type} value={value} minLength={minLength} maxLength={maxLength} pattern={pattern} placeholder={placeholder} onChange={event => onChange(event.target.value)} className="mt-1 block w-full rounded-lg border p-2.5 font-normal dark:bg-slate-900" />{help && <span className="mt-1 block text-xs font-normal text-slate-500">{help}</span>}</label>; }
