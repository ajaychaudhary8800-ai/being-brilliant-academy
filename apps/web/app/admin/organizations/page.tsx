...
import { CheckCircle2, Circle, ClipboardCheck, Edit3, Mail, Plus, Search, School, Trash2, X } from "lucide-react";
...
type SaaSPlan = { code: string; name: string; trialDays: number; isActive: boolean; entitlements: Record<string, boolean> };
type OnboardingProgress = "NOT_STARTED" | "IN_PROGRESS" | "COMPLETE" | "NOT_REQUIRED";
type OnboardingSnapshot = {
  organization: { id: string; slug: string; name: string; subscriptionStatus: string; subscriptionPlan: string };
  administrator: { id: string; name: string; email: string; emailVerifiedAt: string | null } | null;
  activeBranches: number;
  onboarding: { dataMigration: OnboardingProgress; training: OnboardingProgress; targetGoLiveDate: string | null; goLiveApproved: boolean; notes: string | null };
  access: { sharedPath: string; customDomain: string | null };
  subscription: { valid: boolean; aligned: boolean; plan: { code: string; name: string; isActive: boolean } | null };
  checklist: { key: string; label: string; complete: boolean; required: boolean }[];
  readiness: { completed: number; total: number; percent: number; readyForGoLive: boolean; status: "BLOCKED" | "IN_PROGRESS" | "READY" };
};
const empty: OrganizationForm = { slug: "", name: "", email: "", adminName: "", adminEmail: "", logoUrl: null, primaryColor: "#1d4ed8", secondaryColor: "#0f172a", timezone: defaultOrganizationTimezone, locale: "en-IN", currency: "INR", academicYearStartMonth: 4, groupLabelType: "BATCH", customGroupLabel: "", subscriptionStatus: "TRIAL", subscriptionPlan: "ESSENTIALS", trialEndsAt: "", subscriptionEndsAt: "", customDomain: "" };
const dateInput = (value: string | null | undefined) => value ? value.slice(0, 10) : "";
const onboardingProgressOptions = [["NOT_STARTED", "Not started"], ["IN_PROGRESS", "In progress"], ["COMPLETE", "Complete"], ["NOT_REQUIRED", "Not required"]] as const;
...
export default function Page() {
  const [rows, setRows] = useState<Organization[]>([]); const [plans, setPlans] = useState<SaaSPlan[]>([]); const [search, setSearch] = useState(""); const [form, setForm] = useState<OrganizationForm>(empty); const [editing, setEditing] = useState<Organization | null>(null); const [show, setShow] = useState(false); const [slugEdited, setSlugEdited] = useState(false); const [saving, setSaving] = useState(false); const [resendingId, setResendingId] = useState<string | null>(null); const [error, setError] = useState(""); const [notice, setNotice] = useState(""); const [logoFile, setLogoFile] = useState<File | null>(null); const [logoError, setLogoError] = useState(""); const [onboardingOrg, setOnboardingOrg] = useState<Organization | null>(null); const [onboarding, setOnboarding] = useState<OnboardingSnapshot | null>(null); const [onboardingLoading, setOnboardingLoading] = useState(false); const [onboardingSaving, setOnboardingSaving] = useState(false);
  ...
  const selectedPlan = plans.find(plan => plan.code === form.subscriptionPlan);
  const planHas = (feature: string) => selectedPlan?.entitlements?.["*"] === true || selectedPlan?.entitlements?.[feature] === true;
  ...
  async function openOnboarding(org: Organization) {
    setOnboardingOrg(org);
    setOnboarding(null);
    setOnboardingLoading(true);
    setError("");
    try {
      const response = await api("/platform/organizations/" + org.id + "/onboarding");
      setOnboarding(response.data as OnboardingSnapshot);
    } catch (cause) {
      setError(errorMessage(cause));
  ...
  function updateOnboarding<K extends keyof OnboardingSnapshot["onboarding"]>(key: K, value: OnboardingSnapshot["onboarding"][K]) {
    setOnboarding(current => current ? { ...current, onboarding: { ...current.onboarding, [key]: value } } : current);
  }
  async function saveOnboarding() {
    if (!onboardingOrg || !onboarding) return;
    setOnboardingSaving(true);
    setError("");
    setNotice("");
    try {
      const response = await api("/platform/organizations/" + onboardingOrg.id + "/onboarding", {
        method: "PATCH",
        body: JSON.stringify(onboarding.onboarding),
      });
      setOnboarding(response.data as OnboardingSnapshot);
      setNotice("Client onboarding status updated.");
    } catch (cause) {
      setError(errorMessage(cause));
    } finally {
