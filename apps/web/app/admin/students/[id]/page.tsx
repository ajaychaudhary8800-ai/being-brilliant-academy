...
function Detail() {
  ...
  const currentPlacement = data?.currentEnrollment ?? null;
  ...
  return <div className="min-h-screen bg-slate-50 dark:bg-slate-950"><Sidebar/><main className="p-5 md:ml-64 md:p-10"><div className="mx-auto max-w-6xl">
    ...
    {error ? <p className="mt-6 rounded-xl bg-red-50 p-4 text-red-700" role="alert">{error}</p> : loading || !data ? <Loader2 className="mt-12 animate-spin"/> : <>
      ...
      <section className="mt-6 rounded-2xl bg-brand-800 p-6 text-white"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-sm opacity-70">{data.admissionNo} · Roll {currentPlacement?.rollNo ?? data.rollNo}</p><h1 className="mt-1 text-3xl font-bold">{data.user.name}</h1><p>{currentPlacement?.course?.title ?? data.course?.title ?? terms.course} · {currentPlacement?.batch?.name ?? data.batch.name} · {data.status}</p></div><button type="button" className="btn border-white/30 bg-white text-brand-800" onClick={() => setTransitionOpen(true)}><Route size={17}/>Academic transition</button></div><p className="mt-4 max-w-2xl text-sm text-blue-100">Use Academic transition for {terms.mode === "SCHOOL" ? "promotion, retention, transfer, graduation or leaving" : "moving, retention, transfer, graduation or exit"}. Use Edit Placement only to correct incorrectly entered current data.</p></section>
      <section className="mt-5 grid gap-4 md:grid-cols-3">{Object.entries({"Gender":data.gender,"Date of Birth":date(data.dateOfBirth),"Father":data.fatherName,"Mother":data.motherName,"Mobile":data.user.phone,"Parent Mobile":data.parentMobile,"Email":data.user.email,"Branch / Campus":currentPlacement?.branch?.branchName ?? data.branch.name,[terms.course]:currentPlacement?.course?.title ?? data.course?.title,[terms.singular]:currentPlacement?.batch?.name ?? data.batch.name,"Session":currentPlacement?.academicSession?.name ?? data.academicSession,"Admission Date":date(data.admissionDate),"Blood Group":data.bloodGroup,"Category":data.category,"Aadhaar":data.aadhaarNo,"Address":data.address,"Remarks":data.remarks}).map(([label,value]) => <div key={label} className="rounded-xl border bg-white p-4 dark:bg-slate-900"><p className="text-xs font-bold uppercase text-slate-400">{label}</p><p className="mt-1 font-semibold">{String(value ?? "—")}</p></div>)}</section>
