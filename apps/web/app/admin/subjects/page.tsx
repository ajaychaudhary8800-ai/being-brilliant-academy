...
function Content() {
  ...
  return <div className="min-h-screen bg-slate-50 dark:bg-slate-950"><Sidebar/><main className="p-5 md:ml-64 md:p-10"><div className="mx-auto max-w-7xl">
    ...
    <section className="mt-5 overflow-hidden rounded-2xl border bg-white"><div className="overflow-x-auto"><table className="w-full min-w-[900px] text-left text-sm"><thead className="bg-slate-50"><tr>{["Subject", "Description", terms.courses, terms.educators, "Academic references", "Review", "Status", "Actions"].map(label => <th className="p-4" key={label}>{label}</th>)}</tr></thead><tbody>{loading ? <tr><td colSpan={8} className="p-16 text-center">Loading…</td></tr> : rows.length ? rows.map(row => <tr className="border-t" key={row.id}><td className="p-4 align-top"><b>{row.name}</b><br/><span className="text-slate-500">{row.code}</span></td><td className="p-4 align-top">{row.description ?? "—"}</td><td className="p-4 align-top">{row._count.courses}</td><td className="p-4 align-top">{row._count.teachers}</td><td className="p-4 align-top">{row._count.teacherAllocations + row._count.timetables + row._count.homeworks + row._count.examinations}</td><td className="p-4 align-top">{row.legacyReviewStatus === "REVIEW_REQUIRED" ? <span className="text-amber-700">Review required</span> : "Confirmed"}</td><td className="p-4 align-top"><button onClick={() => void toggle(row)} className={row.status === "ACTIVE" ? "text-emerald-700" : "text-slate-500"}>{row.status}</button></td><td className="p-4 align-top"><button className="icon" aria-label="Edit" onClick={() => void open("edit", row)}><Edit3 size={16}/></button>{user?.role === "SUPER_ADMIN" && row.legacyReviewStatus === "REVIEW_REQUIRED" && <button className="icon text-amber-700" aria-label="Clean up legacy subject" title="Clean up redundant legacy mappings" onClick={() => void open("merge", row)}><GitMerge size={16}/></button>}{row.status === "INACTIVE" && <button className="icon text-red-600" aria-label="Delete" onClick={() => void open("delete", row)}><Trash2 size={16}/></button>}</td></tr>) : <tr><td colSpan={8} className="p-16 text-center"><BookOpen className="mx-auto mb-2 text-slate-300"/>No subjects found.</td></tr>}</tbody></table></div><footer className="flex flex-col gap-3 border-t p-4 sm:flex-row sm:items-center sm:justify-between"><span>{meta.total} subjects</span><div className="flex items-center gap-3"><button disabled={page <= 1} onClick={() => setPage(value => value - 1)}><ChevronLeft/></button>{page}/{meta.totalPages}<button disabled={page >= meta.totalPages} onClick={() => setPage(value => value + 1)}><ChevronRight/></button></div></footer></section>
