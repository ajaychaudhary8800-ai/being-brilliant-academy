...
function Content() {
  ...
  return <div className="min-h-screen bg-slate-50"><Sidebar/><main className="p-5 md:ml-64 md:p-10"><div className="mx-auto max-w-6xl">
    ...
    <section className="mt-6 grid gap-4 rounded-2xl border bg-white p-5 md:grid-cols-[1fr_auto]"><label>Assessment<select className="field" value={selected} onChange={event => setSelected(event.target.value)}><option value="">Select assessment</option>{exams.map(item => <option key={item.id} value={item.id}>{item.name} — {item.subject.name} — {item.batch.name}</option>)}</select></label><div className="flex flex-wrap items-end gap-2"><label className={`btn cursor-pointer bg-brand-700 text-white ${paperBusy ? "pointer-events-none opacity-50" : ""}`}>{paperBusy ? <Loader2 className="animate-spin" size={17}/> : <Upload size={17}/>}Upload / Replace Paper<input disabled={paperBusy} className="hidden" type="file" accept=".pdf,.jpg,.jpeg,.png,.doc,.docx,application/pdf,image/jpeg,image/png,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document" onChange={event => void uploadPaper(event.target.files?.[0] ?? null)}/></label>{exam?.questionPaper && <button disabled={paperBusy} className="btn text-red-600" onClick={() => void removePaper()}><Trash2 size={16}/>Remove</button>}</div>{exam?.questionPaper && <p className="text-sm md:col-span-2">Published paper: <button className="font-bold text-brand-700" onClick={() => void download(`/examinations/${exam.id}/question-paper`, exam.questionPaper!.fileName)}>{exam.questionPaper.fileName}</button>{exam._count.answerSheets > 0 && <span className="ml-2 text-slate-500">Locked because submissions exist</span>}</p>}<p className="text-xs text-slate-500 md:col-span-2">PDF, JPG, JPEG, PNG, DOC or DOCX. Maximum 10 MB. Replacement and removal are blocked after submissions to preserve history.</p></section>
