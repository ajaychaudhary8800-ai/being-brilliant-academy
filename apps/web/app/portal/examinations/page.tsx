...
function Content() {
  ...
  async function paper(exam: Exam) {
    const response = await fetch(`${API}/exam-workflow/examinations/${exam.id}/question-paper`, { headers: { Authorization: `Bearer ${getAccessToken() ?? ""}` } });
    if (!response.ok) return setError((await response.json()).error?.message ?? "Paper unavailable");
