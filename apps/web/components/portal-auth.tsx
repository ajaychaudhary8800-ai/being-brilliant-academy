...
export function PortalLogin({ portal: portalKey }: { portal: AuthPortal }) {
  ...
  const { brand, hostBound, resolveWorkspace } = useTenantBranding();
  const router = useRouter();
  const [email, setEmail] = useState("");
  const [password, setPassword] = useState("");
  const [organization, setOrganization] = useState(brand.slug);
  const [workspaceEdited, setWorkspaceEdited] = useState(false);
  const [rememberMe, setRememberMe] = useState(true);
  const [showPassword, setShowPassword] = useState(false);
  const [error, setError] = useState("");
  const [submitting, setSubmitting] = useState(false);
  useEffect(() => { if (!workspaceEdited) setOrganization(brand.slug); }, [brand.slug, workspaceEdited]);
  useEffect(() => {
    if (hostBound || workspaceEdited) return;
    const queryWorkspace = new URLSearchParams(window.location.search).get("workspace")?.trim().toLowerCase().replace(/[^a-z0-9-]/g, "") ?? "";
    if (queryWorkspace.length < 2) return;
    setWorkspaceEdited(true);
    setOrganization(queryWorkspace);
    void resolveWorkspace(queryWorkspace);
  }, [hostBound, resolveWorkspace, workspaceEdited]);
  ...
  return <main id="main-content" className={`grid min-h-screen lg:grid-cols-[.9fr_1.1fr] bg-gradient-to-br ${portal.surface}`}><aside className={`hidden p-12 text-white lg:flex lg:flex-col lg:justify-between ${portal.accent}`}><Brand compact /><div><Icon size={56} /><p className="mt-8 text-sm font-black uppercase tracking-[.2em] opacity-75">{portal.audience}</p><h1 className="mt-3 text-5xl font-black">{portal.title}</h1><p className="mt-5 max-w-lg text-lg leading-8 opacity-85">{portal.description}</p></div><p className="text-sm opacity-70">Secure · Organization-aware · Role protected</p></aside><section className="grid place-items-center px-5 py-10"><div className="w-full max-w-md"><div className="flex items-center justify-between lg:hidden"><Brand compact /><Link href="/login" className="text-sm font-bold text-slate-600">Change portal</Link></div><div className="mt-8 rounded-3xl border border-white/80 bg-white p-7 shadow-2xl shadow-slate-300/40 sm:p-9 lg:mt-0"><div className={`grid h-12 w-12 place-items-center rounded-2xl text-white ${portal.accent}`}><Icon size={23} /></div><h2 className="mt-6 text-3xl font-black">{brand.loginHeadline}</h2><p className="mt-2 text-sm text-slate-500">{brand.loginSubheadline ?? `Sign in to your ${portal.title.toLowerCase()}.`}</p><form onSubmit={submit} className="mt-7 space-y-4"><label className="block text-sm font-semibold">Workspace<input required readOnly={hostBound} value={organization} onChange={(event) => { if (hostBound) return; setWorkspaceEdited(true); setOrganization(event.target.value.toLowerCase().replace(/[^a-z0-9-]/g, "")); }} onBlur={() => { if (!hostBound) void resolveWorkspace(organization); }} autoComplete="organization" className={`mt-1.5 w-full rounded-xl border border-slate-300 px-3 py-2.5 outline-none focus:ring-2 ${portal.ring} ${hostBound ? "bg-slate-100 text-slate-600" : ""}`} />{hostBound ? <span className="mt-1 block text-xs font-normal text-slate-500">Workspace is fixed by this organization URL.</span> : null}</label><label className="block text-sm font-semibold">Email address<input required type="email" autoComplete="email" value={email} onChange={(event) => setEmail(event.target.value)} className={`mt-1.5 w-full rounded-xl border border-slate-300 px-3 py-2.5 outline-none focus:ring-2 ${portal.ring}`} /></label><label className="block text-sm font-semibold">Password<span className="relative mt-1.5 block"><input required type={showPassword ? "text" : "password"} minLength={8} autoComplete="current-password" value={password} onChange={(event) => setPassword(event.target.value)} className={`w-full rounded-xl border border-slate-300 px-3 py-2.5 pr-12 outline-none focus:ring-2 ${portal.ring}`} /><button type="button" onClick={() => setShowPassword((value) => !value)} aria-label={showPassword ? "Hide password" : "Show password"} className="absolute right-2 top-1/2 -translate-y-1/2 rounded-lg p-2 text-slate-500 hover:bg-slate-100">{showPassword ? <EyeOff size={18} /> : <Eye size={18} />}</button></span></label><div className="flex items-center justify-between text-sm"><label className="flex items-center gap-2"><input type="checkbox" checked={rememberMe} onChange={(event) => setRememberMe(event.target.checked)} /> Remember me</label><Link href={`/forgot-password/${portalKey}`} className="font-bold text-brand-700">Forgot password?</Link></div>{error && <p role="alert" className="rounded-xl bg-red-50 p-3 text-sm font-medium text-red-700">{error}</p>}<button disabled={submitting} className={`w-full rounded-xl py-3 font-bold text-white disabled:opacity-60 ${portal.accent}`}>{submitting ? "Verifying account…" : `Sign in to ${portal.title}`}</button></form><Link href="/login" className="mt-6 flex items-center justify-center gap-2 text-sm font-bold text-slate-500"><ArrowLeft size={15} /> Choose another portal</Link></div></div></section></main>;
}

export function PortalForgotPassword({ portal: portalKey }: { portal: AuthPortal }) {
  const portal = portals[portalKey]; const Icon = portal.icon; const { requestPasswordReset } = useAuth(); const { brand, hostBound, resolveWorkspace } = useTenantBranding();
  const [email, setEmail] = useState(""); const [organization, setOrganization] = useState(brand.slug); const [workspaceEdited, setWorkspaceEdited] = useState(false); const [message, setMessage] = useState(""); const [error, setError] = useState(""); const [submitting, setSubmitting] = useState(false);
  useEffect(() => { if (!workspaceEdited) setOrganization(brand.slug); }, [brand.slug, workspaceEdited]);
  useEffect(() => {
    if (hostBound || workspaceEdited) return;
    const queryWorkspace = new URLSearchParams(window.location.search).get("workspace")?.trim().toLowerCase().replace(/[^a-z0-9-]/g, "") ?? "";
