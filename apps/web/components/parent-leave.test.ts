...
import { parentLeaveChildSummary, parentLeaveSelection, type ParentLeaveChild } from "./parent-leave";
...
test("child summaries identify the leave subject without exposing internal user ids", () => {
  assert.equal(parentLeaveChildSummary(child("one")), "Student one · ADM-one · Class 5 · Morning");
});
