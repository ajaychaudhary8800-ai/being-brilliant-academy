...
async function acknowledgeNotice(noticeId: string) {
  ...
  if (!response.ok) {
    ...
    if (response.status === 403) throw new Error("You are not allowed to acknowledge this notice.");
    if (response.status === 404) throw new Error("This notice is no longer available.");
    throw new Error("Unable to acknowledge this notice. Please try again.");
  }
...
function Badge({ value }: { value: string }) {
  ...
  const warning = ["PENDING", "PARTIAL", "LATE", "LATE_SUBMITTED", "UNDER_REVIEW", "SCHEDULED"].includes(value);
  return <span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-bold ${positive ? "bg-emerald-50 text-emerald-700" : warning ? "bg-amber-50 text-amber-700" : "bg-slate-100 text-slate-700"}`}>{readable(value)}</span>;
}
