...
import { buildBulkResultsCsv, buildBulkTransitionPayload, bulkTransitionNeedsDestination, duplicateBulkRollNumbers, initialBulkRollNumbers, localResultsFilename, normalizeBulkRollNumbers, type BulkStudent, type BulkTransitionResponse } from "./student-bulk-transition";
...
export default function StudentBulkTransitionDialog({ open, students, onCancel, onCompletedClose, onRefresh }: Props) {
  ...
  const confirm = async () => {
    ...
    try {
      ...
      if (!json?.data || !Array.isArray(json.data.results)) throw new Error("The bulk transition response was invalid.");
      const processed: BulkTransitionResponse = { httpStatus: http.status, data: json.data };
