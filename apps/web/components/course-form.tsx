...
export default function CourseForm({ courseId }: { courseId?: string }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      const availableBranches = branchJson.data ?? [];
      setExams(taxonomy.data.competitiveExams); setSkills(taxonomy.data.skillCategories); setBranches(availableBranches);
      if (!courseId && user?.role === "BRANCH_ADMIN" && availableBranches.length) setForm(current => ({ ...current, branchId: current.branchId || availableBranches[0].id }));
