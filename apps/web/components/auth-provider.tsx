...
export function AuthGate({ children, roles }: { children: React.ReactNode; roles?: AppRole[] }) { const { user, loading } = useAuth(); const router = useRouter(); const pathname = usePathname(); const roleKey = roles?.join(",") ?? ""; const accountantFeeAccess = user?.role === "ACCOUNTANT" && (pathname === "/admin/fees" || pathname === "/admin/fee-defaulters"); const denied = Boolean(roles && user && !roles.includes(user.role) && !accountantFeeAccess); useEffect(() => { if (!loading && (!user || denied)) router.replace(!user ? `/login?next=${encodeURIComponent(pathname)}` : "/dashboard"); }, [loading, user, denied, roleKey, router, pathname]); if (loading || !user || denied) return <main className="grid min-h-screen place-items-center bg-slate-50 text-sm text-slate-500 dark:bg-slate-950">Loading your secure workspace…</main>; return <>{children}</>; }
