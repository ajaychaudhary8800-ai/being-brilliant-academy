...
export default function Sidebar() {
  ...
  return <>
    <aside className="fixed inset-y-0 left-0 z-40 hidden w-64 flex-col border-r border-slate-200 bg-white px-3 py-6 dark:border-slate-800 dark:bg-slate-950 md:flex">
      <Link href={user?.billingRecovery ? "/admin/subscription" : "/admin"} className="flex items-center gap-3 px-3 font-bold tracking-tight text-brand-700">{brand.logoUrl ? <Image unoptimized src={brand.logoUrl} alt="" width={38} height={38} className="h-10 w-10 rounded-lg bg-white object-contain" /> : <span className="grid h-10 w-10 place-items-center rounded-lg bg-brand-700 text-white"><School size={20}/></span>}<span className="min-w-0" title={brand.appName}><span className="block max-w-[160px] whitespace-normal break-words text-sm leading-tight">{brand.appName}</span><span className="mt-1 block truncate text-[10px] font-semibold uppercase tracking-[0.2em] text-slate-400">{brand.portalName}</span></span></Link>
