...
type Exam = {
  ...
  batch: { name: string };
  subject: { name: string };
  questionPaper: { fileName: string; publishedAt: string | null } | null;
...
export function ExaminationWorkflowContent({ teacherView = false }: { teacherView?: boolean }) {
  ...
  const load = useCallback(async () => {
    setBusy(true);
    try {
      const result = await fetch(`${API}/exam-workflow/examinations`, {
        headers: headers(),
      }).then(body);
  ...
  const loadSheets = useCallback(async () => {
    ...
    try {
      ...
      setSheets(value.data);
      setError("");
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : "Unable to load answer sheets");
    }
  }, [selected]);
  ...
  async function uploadPaper(file: File | null) {
    ...
    setError("");
    try {
      await fetch(`${API}/exam-workflow/examinations/${selected}/question-paper`, {
        method: "PUT",
        headers: headers(),
        body: JSON.stringify({
          fileName: file.name,
      ...
      setNotice("Question paper uploaded and published");
      await load();
    } catch (cause) {
  ...
  async function removePaper() {
    ...
    try {
      ...
      setNotice("Question paper removed");
      await load();
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : "Unable to remove question paper");
    } finally {
      setPaperBusy(false);
    }
  }
  ...
  async function evaluate(sheet: Sheet, marks: string, remarks: string, finalize: boolean) {
    if (marks === "") {
      setError("Enter marks before saving an evaluation.");
      return;
    }
    try {
      await fetch(`${API}/exam-workflow/answer-sheets/${sheet.id}/evaluation`, {
        method: "PATCH",
        headers: headers(),
        body: JSON.stringify({
      ...
      setNotice(finalize ? "Evaluation finalized" : "Evaluation saved as under review");
      await loadSheets();
    } catch (cause) {
      ...
      <main className={`p-5 md:p-10 ${teacherView ? "" : "md:ml-64"}`}>
        <div className="mx-auto max-w-6xl">
          <header>
            ...
            <p className="text-slate-500">Secure uploads, review, marks, feedback and finalization.</p>
          </header>
          ...
          <section className="mt-6 grid gap-4 rounded-2xl border bg-white p-5 md:grid-cols-[1fr_auto]">
            <label>
              Assessment
              <select className="field" value={selected} onChange={(event) => setSelected(event.target.value)}>
                ...
                {exams.map((item) => (
                  <option key={item.id} value={item.id}>
                    {item.name} — {item.subject.name} — {item.batch.name}
                  </option>
                ))}
              </select>
            </label>
            <div className="flex flex-wrap items-end gap-2">
              <label className={`btn cursor-pointer bg-brand-700 text-white ${paperBusy ? "pointer-events-none opacity-50" : ""}`}>
                {paperBusy ? <Loader2 className="animate-spin" size={17} /> : <Upload size={17} />}
            ...
            {exam?.questionPaper && (
              <p className="text-sm md:col-span-2">
                Published paper:{" "}
                <button className="font-bold text-brand-700" onClick={() => void download(`/examinations/${exam.id}/question-paper`, exam.questionPaper!.fileName)}>
                  {exam.questionPaper.fileName}
                </button>
                {exam._count.answerSheets > 0 && <span className="ml-2 text-slate-500">Locked because submissions exist</span>}
              </p>
            )}
            <p className="text-xs text-slate-500 md:col-span-2">PDF, JPG, JPEG, PNG, DOC or DOCX. Maximum 10 MB. Replacement and removal are blocked after submissions to preserve history.</p>
          </section>
          <section className="mt-5 rounded-2xl border bg-white p-5">
            <h2 className="text-xl font-bold">Student submissions ({sheets.length})</h2>
            {busy ? (
              <Loader2 className="mx-auto my-12 animate-spin" />
            ) : sheets.length ? (
              <div className="mt-4 space-y-3">
                {sheets.map((sheet) => (
                  <Evaluation key={sheet.id} sheet={sheet} maximum={exam?.maximumMarks ?? 0} save={evaluate} download={() => void download(`/answer-sheets/${sheet.id}/file`, sheet.fileName)} />
                ))}
              </div>
            ) : (
...
function Evaluation({ sheet, maximum, save, download }: { sheet: Sheet; maximum: number; save: (sheet: Sheet, marks: string, remarks: string, finalize: boolean) => void; download: () => void }) {
  const [marks, setMarks] = useState(sheet.marksObtained ?? ""),
    [remarks, setRemarks] = useState(sheet.teacherRemarks ?? "");
  useEffect(() => {
    setMarks(sheet.marksObtained ?? "");
    setRemarks(sheet.teacherRemarks ?? "");
  }, [sheet.marksObtained, sheet.teacherRemarks]);
  const finalized = Boolean(sheet.finalizedAt);
  return (
    <article className="grid gap-3 rounded-xl border p-4 md:grid-cols-[1fr_120px_2fr_auto]">
      <div>
        <b>{sheet.student.user.name}</b>
        <p className="text-xs text-slate-500">
          {sheet.student.admissionNo} · {finalized ? "FINALIZED" : sheet.status}
          {sheet.isLate ? " · LATE" : ""}
        </p>
      ...
      <label>
        Marks
        <input disabled={finalized} className="field" type="number" min="0" max={maximum} value={marks} onChange={(event) => setMarks(event.target.value)} />
      </label>
      <label>
        Feedback
        <textarea disabled={finalized} className="field" value={remarks} onChange={(event) => setRemarks(event.target.value)} />
      </label>
      <div className="flex flex-col gap-2">
        <button disabled={finalized || marks === ""} className="btn disabled:opacity-50" onClick={() => save(sheet, marks, remarks, false)}>
          Save Review
        </button>
        <button disabled={finalized || marks === ""} className="btn bg-brand-700 text-white disabled:opacity-50" onClick={() => save(sheet, marks, remarks, true)}>
