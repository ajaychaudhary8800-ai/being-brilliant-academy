...
test("Core LMS HTTP routes enforce allocations, learner scope, branch scope and metadata protection", async t => {
  ...
  const target = () => ({ id: LESSON, branchId: targetBranch, courseId: targetCourse, batchId: targetBatch, subjectId: targetSubject, teacherId: targetTeacher, status: targetStatus, durationSeconds: 600, videoData: Buffer.from("video"), videoMime: "video/mp4", videoSize: 5, videoUrl: null, _count: { progress: 0 } });
