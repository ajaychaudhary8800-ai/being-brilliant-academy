...
router.post("/learning/tests/:id/start", allow(Role.STUDENT), async (req: AuthRequest, res) => {
  ...
  if (!learner) throw learningDenied();
  if (test.batchId) {
    const batch = await prisma.batch.findFirst({ where: { organizationId: req.auth!.organizationId, id: test.batchId }, select: { branchId: true, courseId: true, academicSessionId: true } });
    const testDate = test.startsAt ? new Date(Date.UTC(test.startsAt.getUTCFullYear(), test.startsAt.getUTCMonth(), test.startsAt.getUTCDate())) : new Date();
