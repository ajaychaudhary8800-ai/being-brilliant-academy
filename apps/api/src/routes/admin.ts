...
router.delete("/teachers/:id", async (req: AuthRequest, res) => { const teacher = await prisma.teacherProfile.findUnique({ where: { id: String(req.params.id) }, select: { id: true, userId: true, branchId: true, user: { select: { avatarUrl: true } }, _count: { select: { allocations: true } } } }); if (!teacher) throw new AppError(404, "TEACHER_NOT_FOUND", "Teacher not found"); await requireBranchAccess(req, teacher.branchId); if (teacher._count.allocations) throw new AppError(409, "TEACHER_HAS_ALLOCATIONS", "Deactivate or remove teacher allocations before deleting this teacher"); await prisma.$transaction([prisma.teacherProfile.delete({ where: { id: teacher.id } }), prisma.user.delete({ where: { id: teacher.userId } })]); await deleteUnusedTeacherPhoto(req.auth!.organizationId, teacher.user.avatarUrl).catch(error => logger.warn({ err: error, teacherId: teacher.id }, "Unable to remove deleted teacher photo")); res.status(204).send(); });
