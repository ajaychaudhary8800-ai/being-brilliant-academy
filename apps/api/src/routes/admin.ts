...
router.delete("/branches/:id", async (req: AuthRequest, res) => { const existing = await prisma.branch.findUnique({ where: { id: String(req.params.id) }, select: { id: true, _count: { select: { students: true, teachers: true, batches: true } } } }); if (!existing) throw new AppError(404, "BRANCH_NOT_FOUND", "Branch not found"); await requireBranchAccess(req, existing.id); if (existing._count.students || existing._count.teachers || existing._count.batches) throw new AppError(409, "BRANCH_IN_USE", "Move students, teachers and batches before deleting this branch"); await prisma.branch.delete({ where: { id: existing.id } }); res.status(204).send(); });
...
router.delete("/teachers/:id", async (req: AuthRequest, res) => { const teacher = await prisma.teacherProfile.findUnique({ where: { id: String(req.params.id) }, select: { id: true, userId: true, branchId: true } }); if (!teacher) throw new AppError(404, "TEACHER_NOT_FOUND", "Teacher not found"); await requireBranchAccess(req, teacher.branchId); await prisma.$transaction([prisma.teacherProfile.delete({ where: { id: teacher.id } }), prisma.user.delete({ where: { id: teacher.userId } })]); res.status(204).send(); });
