...
async function ensureSubject(req:AuthRequest,data:z.infer<typeof input>){const organizationId=req.auth!.organizationId,name=data.subjectName.trim();let subject=await prisma.subject.findFirst({where:{organizationId,name:{equals:name,mode:"insensitive"}},select:{id:true,name:true}});if(!subject){const code=`TA-${createHash("sha256").update(`${organizationId}|${name.toLowerCase()}`).digest("hex").slice(0,12).toUpperCase()}`;try{subject=await prisma.subject.create({data:{organizationId,name,code},select:{id:true,name:true}})}catch(error){if(!isDuplicate(error))throw error;subject=await prisma.subject.findFirst({where:{organizationId,OR:[{code},{name:{equals:name,mode:"insensitive"}}]},select:{id:true,name:true}});if(!subject)throw error}}const link=await prisma.courseSubject.findUnique({where:{courseId_subjectId:{courseId:data.courseId,subjectId:subject.id}},select:{isActive:true}});if(!link)try{await prisma.courseSubject.create({data:{organizationId,courseId:data.courseId,subjectId:subject.id,isActive:true}})}catch(error){if(!isDuplicate(error))throw error}return subject}
