...
const branchIdsForActor = async (req: AuthRequest) => req.auth!.role === Role.SUPER_ADMIN
  ? null
  : (await prisma.branchUser.findMany({ where: { organizationId: req.auth!.organizationId, userId: req.auth!.userId }, select: { branchId: true } })).map(item => item.branchId);
...
router.post("/users/accountants", allow(Role.SUPER_ADMIN), async (req: AuthRequest, res) => {
  ...
  try {
    const result = await prisma.$transaction(async tx => {
      ...
      await tx.auditLog.create({ data: { organizationId: req.auth!.organizationId, actorId: req.auth!.userId, action: "USER_CREATED", entity: "User", entityId: user.id, metadata: { role: Role.ACCOUNTANT, branchCount: input.branchIds.length } } });
      return user;
    });
    let setup: unknown = null;
    try { setup = await issueAccountSetup(result); await prisma.auditLog.create({ data: { organizationId: req.auth!.organizationId, actorId: req.auth!.userId, action: "PASSWORD_RESET_REQUESTED", entity: "User", entityId: result.id, metadata: { role: Role.ACCOUNTANT, delivery: (setup as any)?.skipped ? "SKIPPED" : "SENT" } } }); } catch { setup = { skipped: true, reason: "EMAIL_DELIVERY_FAILED" }; }
...
async function validateStudents(req: AuthRequest, students: Array<{ studentId: string; relationship: string }>) {
  ...
  const records = await prisma.studentProfile.findMany({ where: { organizationId: req.auth!.organizationId, id: { in: ids } }, select: { id: true, branchId: true, status: true, user: { select: { isActive: true } } } });
...
router.post("/users/parents", async (req: AuthRequest, res) => {
  ...
  try {
    const result = await prisma.$transaction(async tx => {
      const user = await tx.user.create({ data: { organizationId: req.auth!.organizationId, name: input.name, email: input.email, phone: input.phone ?? null, isActive: input.isActive, role: Role.PARENT, passwordHash: await bcrypt.hash(crypto.randomBytes(32).toString("base64url"), 12) }, select: { id: true, organizationId: true, name: true, email: true, role: true, isActive: true } });
      await tx.parentStudent.createMany({ data: input.students.map(item => ({ organizationId: req.auth!.organizationId, parentId: user.id, studentId: item.studentId, relationship: item.relationship })) });
      await tx.auditLog.create({ data: { organizationId: req.auth!.organizationId, actorId: req.auth!.userId, action: "USER_CREATED", entity: "User", entityId: user.id, metadata: { role: Role.PARENT, studentCount: input.students.length } } });
      for (const item of input.students) await tx.auditLog.create({ data: { organizationId: req.auth!.organizationId, actorId: req.auth!.userId, action: "PARENT_STUDENT_LINKED", entity: "ParentStudent", entityId: user.id, metadata: { studentId: item.studentId, relationship: item.relationship } } });
      return user;
    });
    let setup: unknown = null; try { setup = await issueAccountSetup(result); await prisma.auditLog.create({ data: { organizationId: req.auth!.organizationId, actorId: req.auth!.userId, action: "PASSWORD_RESET_REQUESTED", entity: "User", entityId: result.id, metadata: { role: Role.PARENT, delivery: (setup as any)?.skipped ? "SKIPPED" : "SENT" } } }); } catch { setup = { skipped: true, reason: "EMAIL_DELIVERY_FAILED" }; }
    res.status(201).json({ data: { ...result, setup } });
  } catch (error) { const mapped = mapUnique(error); if (mapped) throw mapped; throw error; }
});
...
router.post("/users/:id/children", async (req: AuthRequest, res) => {
  ...
  try { const link = await prisma.$transaction(async tx => { const created = await tx.parentStudent.create({ data: { organizationId: req.auth!.organizationId, parentId: parent.id, studentId: input.studentId, relationship: input.relationship } }); await tx.auditLog.create({ data: { organizationId: req.auth!.organizationId, actorId: req.auth!.userId, action: "PARENT_STUDENT_LINKED", entity: "ParentStudent", entityId: parent.id, metadata: { studentId: input.studentId, relationship: input.relationship } } }); return created; }); res.status(201).json({ data: link }); } catch (error) { const mapped = mapUnique(error); if (mapped) throw mapped; throw error; }
});

router.delete("/users/:id/children/:studentId", async (req: AuthRequest, res) => {
  const parent = await loadTarget(req, String(req.params.id)); if (parent.role !== Role.PARENT) throw new AppError(409, "PARENT_ROLE_REQUIRED", "The selected account is not a Parent");
  const student = await prisma.studentProfile.findFirst({ where: { organizationId: req.auth!.organizationId, id: String(req.params.studentId) }, select: { id: true, branchId: true } }); if (!student) throw new AppError(404, "STUDENT_NOT_FOUND", "Student not found");
  const link = await prisma.parentStudent.findFirst({ where: { organizationId: req.auth!.organizationId, parentId: parent.id, studentId: student.id } }); if (!link) throw new AppError(404, "LINK_NOT_FOUND", "Parent-student link not found");
  ...
  await prisma.$transaction([prisma.parentStudent.delete({ where: { parentId_studentId: { parentId: parent.id, studentId: student.id } } }), prisma.auditLog.create({ data: { organizationId: req.auth!.organizationId, actorId: req.auth!.userId, action: "PARENT_STUDENT_UNLINKED", entity: "ParentStudent", entityId: parent.id, metadata: { studentId: student.id, relationship: link.relationship } } })]); res.status(204).send();
});
...
router.put("/users/:id/accountant-branches", allow(Role.SUPER_ADMIN), async (req: AuthRequest, res) => {
  ...
  await prisma.$transaction(async tx => {
    ...
    await tx.auditLog.create({ data: { organizationId: req.auth!.organizationId, actorId: req.auth!.userId, action: "ACCOUNTANT_BRANCHES_CHANGED", entity: "User", entityId: target.id, metadata: { branchCount: branchIds.length, branchIds } } });
  });
