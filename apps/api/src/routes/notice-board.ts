...
router.post("/notices/:noticeId/acknowledge", async (req: AuthRequest, res) => { const eligibility = await recipientConstraints(req), notice = await prisma.announcement.findFirst({ where: { id: id.parse(req.params.noticeId), kind: "NOTICE", requiresAcknowledgement: true, isArchived: false, deletedAt: null, ...eligibility }, select: { id: true } }); if (!notice) throw new AppError(404, "NOTICE_NOT_FOUND", "Notice not found"); res.status(201).json({ data: await prisma.announcementRead.upsert({ where: { announcementId_userId: { announcementId: notice.id, userId: req.auth!.userId } }, update: {}, create: { announcementId: notice.id, userId: req.auth!.userId } }) }); });
