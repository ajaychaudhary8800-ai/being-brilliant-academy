...
test("tenant generated documents and notifications do not hard-code Being Brilliant branding", async () => {
  const [certificates, students, completion, premium, attendance] = await Promise.all([
    ...
    readFile(new URL("../routes/attendance-reports.ts", import.meta.url), "utf8"),
  ]);

  for (const source of [certificates, students, completion, premium, attendance]) {
  ...
  assert.doesNotMatch(attendance, /\?\.name \?\? "Being Brilliant Academy"/);
