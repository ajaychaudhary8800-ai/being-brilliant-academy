...
Store email/password pairs for `SUPER_ADMIN`, `BRANCH_ADMIN`, `ACCOUNTANT`, `TEACHER`, `STUDENT`, `PARENT`, and `EMPLOYEE` as repository Actions secrets. The nightly workflow fixes the target to `https://main-staging.beingbrilliantedu.com` and the dedicated synthetic organization slug to `automated-qa-academy`.
